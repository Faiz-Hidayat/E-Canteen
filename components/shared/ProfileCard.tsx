    ...
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <h2 className="truncate text-lg font-extrabold text-gray-800">{name}</h2>
              <Badge variant="outline" className={cn('shrink-0 text-[10px] font-bold', badge.className)}>
                {badge.label}
              </Badge>
            </div>
      ...
      <motion.div
        ...
        <svg className="absolute inset-0 h-full w-full opacity-10" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="polkadot-profile" x="0" y="0" width="20" height="20" patternUnits="userSpaceOnUse">
              <circle cx="2" cy="2" r="2" fill="currentColor" />
            </pattern>
          </defs>
          <rect x="0" y="0" width="100%" height="100%" fill="url(#polkadot-profile)" />
        </svg>
        ...
        <div className="relative z-10">
          <p className="mb-1 text-sm font-bold tracking-wide text-white/90">Saldo Jajanmu</p>
          <div className="flex items-baseline gap-1">
            <span className="text-xl font-bold">Rp</span>
            <NumberTicker value={balance} className="text-4xl font-extrabold tracking-tight" />
          </div>
        </div>
      </motion.div>
    </div>
