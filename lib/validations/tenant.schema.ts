...
export const CreateTenantSchema = z.object({
  ...
  adminEmail: z.string({ message: 'Email admin wajib diisi.' }).email('Format email tidak valid.'),
  adminName: z
    .string({ message: 'Nama admin wajib diisi.' })
    .min(2, 'Nama admin minimal 2 karakter.')
    .max(100, 'Nama admin maksimal 100 karakter.'),
  adminPassword: z.string({ message: 'Password admin wajib diisi.' }).min(6, 'Password minimal 6 karakter.'),
...
export const UpdateTenantSchema = z.object({
  ...
  name: z.string().min(2, 'Nama stan minimal 2 karakter.').max(100, 'Nama stan maksimal 100 karakter.').optional(),
  description: z.string().max(500, 'Deskripsi maksimal 500 karakter.').optional().or(z.literal('')),
});
